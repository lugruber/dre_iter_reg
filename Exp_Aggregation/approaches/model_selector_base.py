...
class ModelSelector(ABC):
    ...
    def __init__(self, manual_filter_lambdas: List[str] = [], train_val_split: float = 0.5, seed: int = 42, dre_method=None):
        self.seed = seed
    ...
    def _set_importance_weights_from_source(
            self, domain_classifier_act: Dict[str, Dict[str, np.ndarray]], dre_method=None):
        ...
        # Change this to adaptively select for each dre-method

        n_s = source_preds_act.shape[0]
        n_t = domain_classifier_act['t_preds'].shape[0]
        self.importance_weights = get_weights_numpy(source_preds_act, n_s, n_t, dre_method=self.dre_method)

        #self.importance_weights = source_preds_act
...
class LinearRegressionSelector(ModelSelector):
    ...
    def __init__(self, manual_filter_lambdas: List[str] = [], dre_method=None):
        super().__init__(manual_filter_lambdas, dre_method=dre_method)
    ...
    def _linear_regression(self, X_pred: np.ndarray, y_labels: np.ndarray) -> Tuple[np.ndarray, np.ndarray]:
        # linear regression
        # X = np.matmul(X_pred.T,X_pred)
        # X = np.linalg.inv(X)
        # X = np.matmul(X, X_pred.T)

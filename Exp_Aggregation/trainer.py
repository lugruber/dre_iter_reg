...
from Exp_Kanamori.models_torch import IteratedKulsif, IterCpeExp, IterLogi, IterSquare
...
class cross_domain_dre(object):
   ...
   This class contain the main training functions for our AdAtime and training of DRE methods
    ...
    def __init__(self, args):
        ...
        self.reg_par = args.reg_par
        self.niter = args.niter_iterkulsif
    ...
    def train(self):
        ...
        for i in scenarios:
            ...
            for run_id in range(self.num_runs):  # specify number of consecutive runs
                ...
                # get DRE method
                if self.dre_method == 'kul':
                    DREEstimator = IteratedKulsif(kernel=self.kernel, rbf_gam=rbf_gam)
                    param_grid = {
                        'reg_par': list(10 ** (-np.arange(start=-4, step=1, stop=7, dtype=float)))}
                elif self.dre_method == 'cpeexp':
                    DREEstimator = IterCpeExp(kernel=self.kernel, rbf_gam=rbf_gam)
                    param_grid = {
                        'reg_par': list(10 ** (-np.arange(start=-4, step=1, stop=7, dtype=float))),
                        'niter': list(np.arange(start=1, step=1, stop=2, dtype=float))}
                elif self.dre_method == 'square':
                    DREEstimator = IterSquare(kernel=self.kernel, rbf_gam=rbf_gam)
                    param_grid = {
                        'reg_par': list(10 ** (-np.arange(start=-4, step=1, stop=7, dtype=float))),
                        'niter': list(np.arange(start=1, step=1, stop=2, dtype=float))}
                elif self.dre_method == 'itercpeexp':
                    DREEstimator = IterCpeExp(kernel=self.kernel, rbf_gam=rbf_gam)
                    param_grid = {
                        'reg_par': list(10 ** (-np.arange(start=-4, step=1, stop=7, dtype=float))),
                        'niter': list(np.arange(start=5, step=5, stop=16, dtype=float))}
                elif self.dre_method == 'log':
                    DREEstimator = IterLogi(kernel=self.kernel, rbf_gam=rbf_gam)
                    param_grid = {
                        'reg_par': list(10 ** (-np.arange(start=-4, step=1, stop=7, dtype=float))),
                        'niter': list(np.arange(start=1, step=1, stop=2, dtype=float))}
                elif self.dre_method == 'iterlog':
                    DREEstimator = IterLogi(kernel=self.kernel, rbf_gam=rbf_gam)
                    param_grid = {
                        'reg_par': list(10 ** (-np.arange(start=-4, step=1, stop=7, dtype=float))),
                        'niter': list(np.arange(start=5, step=5, stop=16, dtype=float))}
                elif self.dre_method == 'itersquare':
                    DREEstimator = IterSquare(kernel=self.kernel, rbf_gam=rbf_gam)
                    param_grid = {
                        'reg_par': list(10 ** (-np.arange(start=-4, step=1, stop=7, dtype=float))),
                        'niter': list(np.arange(start=5, step=5, stop=16, dtype=float))}
                elif self.dre_method == 'iterkul':
                    DREEstimator = IteratedKulsif(kernel=self.kernel, rbf_gam=rbf_gam)
                    param_grid = {
                        'reg_par': list(10 ** (-np.arange(start=-4, step=1, stop=7, dtype=float))),
                        'niter': list(np.arange(start=5, step=5, stop=16, dtype=float))
            ...
            pred_file = os.path.join(self.exp_log_dir, f'iwv_pred_dataset_{ds_name}_{self.dre_method}_cv.npz')
            best_params_dir = os.path.join(self.exp_log_dir, f'best_params_{ds_name}_{self.dre_method}_cv.npz')
            os.makedirs(self.exp_log_dir, exist_ok=True)
            np.savez(pred_file, iwv_predictions)
            np.savez(best_params_dir, best_params)
